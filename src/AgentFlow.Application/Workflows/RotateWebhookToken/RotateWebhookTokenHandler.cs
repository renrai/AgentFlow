...
public sealed class RotateWebhookTokenHandler(
    IWorkflowRepository workflows,
    ITenantRepository tenants,
    IUnitOfWork unitOfWork,
    IClock clock)
{
    public async Task<RotateWebhookTokenResult> HandleAsync(
        RotateWebhookTokenCommand command,
        CancellationToken cancellationToken)
    {
        ...
        workflow.RotateWebhookToken(clock.UtcNow);
        await unitOfWork.SaveChangesAsync(cancellationToken).ConfigureAwait(false);

        return new RotateWebhookTokenResult(workflow.Id, workflow.WebhookToken);
    }
}
